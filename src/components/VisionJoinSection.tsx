...
const inputCls =
  "w-full rounded-xl border border-[#E0D2CC] bg-white px-4 py-3.5 text-brand-nero placeholder:text-brand-grigio-light transition-all focus:border-[#C0471A] focus:outline-none focus:ring-[3px] focus:ring-[#C0471A]/12";

const labelCls = "mb-2 block text-[0.9rem] font-medium text-brand-nero";
...
export default function VisionJoinSection() {
  ...
  return (
    <section
      ...
      className={`${bricolage.variable} ${hanken.variable} scroll-mt-24 bg-[#FAECE7] py-[72px] md:py-[120px]`}
      ...
      <div className="mx-auto grid max-w-[1100px] grid-cols-1 items-start gap-10 px-6 md:grid-cols-2 md:gap-16">
        <div ref={copyRef}>
          ...
          <h2
            ...
            className="mt-4 text-balance leading-[1.1] text-[#1A1A1A]"
            style={{
          ...
          >
            La community è il progetto vero.
          </h2>
          <p
            className="mt-6 text-pretty text-[#1A1A1A]/80"
            style={{
        ...
        <div ref={formRef}>
          <div className="rounded-3xl bg-white p-7 shadow-[0_8px_40px_rgba(0,0,0,0.06)] md:p-10">
            {success ? (
              <div className="py-8 text-center">
                <p
                  className="text-xl font-semibold text-brand-nero"
              ...
              <form onSubmit={handleSubmit} noValidate>
                ...
                <div className="mb-5">
                  <label htmlFor="vision-tel" className={labelCls} style={{ fontFamily: "var(--font-vision-join-body)" }}>
                    Telefono <span className="font-normal text-brand-grigio">(opzionale)</span>
                  </label>
                  <input
                    id="vision-tel"
                    type="tel"
                ...
                <label className="mb-6 flex cursor-pointer items-start gap-3">
                  <input
                    ...
                    className="mt-1 h-4 w-4 shrink-0 rounded border-[#E0D2CC] text-[#C0471A] focus:ring-[#C0471A]/20"
                  />
                  <span
                    className="text-[0.85rem] leading-snug text-[#1A1A1A]/70"
                    style={{ fontFamily: "var(--font-vision-join-body)" }}
                  >
                    Ho letto e accetto la{" "}
                    <Link href="/privacy-policy" className="text-[#C0471A] underline underline-offset-2 hover:text-[#A93D16]">
                      privacy policy
                    </Link>
                  </span>
                </label>
                ...
                {error ? (
                  <p className="mb-4 text-sm text-red-700" role="alert" aria-live="polite">
                    {error}
                  </p>
                ) : null}
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="group mt-2 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#C0471A] px-8 py-4 text-base font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#A93D16] disabled:cursor-not-allowed disabled:opacity-60 md:w-auto"
                  style={{ fontFamily: "var(--font-vision-join-body)" }}
                >
                  {submitting ? "Invio in corso..." : "Raccontati"}
                  {!submitting ? (
